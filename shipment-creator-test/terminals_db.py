...
CREATE TABLE IF NOT EXISTS terminals (
    ...
    scraped_at    REAL,
    updated_at    REAL,
    edited_at     REAL                     -- last MANUAL edit via the Terminals UI (NULL = never
                                           -- hand-edited); marks rows changed locally, e.g. for a
                                           -- future push back to SuperDispatch
);
...
def init_db() -> None:
    ...
    with connect() as conn:
        ...
        if "edited_at" not in cols:
            conn.execute("ALTER TABLE terminals ADD COLUMN edited_at REAL")
        conn.commit()
    _initialized = True
...
# The columns a user may hand-edit in the Terminals UI. Everything else (source, links,
# raw_json, the SD ids) is system-owned and never touched by a manual edit.
EDITABLE_FIELDS = ("name", "address", "city", "state", "zip",
                   "contact_name", "contact_phone", "carrier_notes")


def update_terminal_fields(sd_id: str, fields: dict) -> Optional[dict]:
    """Apply a MANUAL edit to one existing terminal (keyed by sd_id) and persist it
    immediately. Only EDITABLE_FIELDS are written; source, links, and the scraped
    raw_json are left intact. name_norm/zip5 are recomputed so the posting lookups stay
    correct, and edited_at is stamped so a later SD push can tell what changed locally.
    Returns the refreshed row, or None if no terminal has that sd_id (or nothing to do)."""
    sd_id = (sd_id or "").strip()
    if not sd_id:
        return None
    _ensure()
    updates = {k: (fields.get(k) or "").strip() for k in EDITABLE_FIELDS if k in fields}
    if not updates:
        return get_by_sd_id(sd_id)
    now = time.time()
    cols, vals = [], []
    for k, v in updates.items():
        cols.append(f"{k}=?"); vals.append(v)
        if k == "name":
            cols.append("name_norm=?"); vals.append(normalize_name(v))
        if k == "zip":
            cols.append("zip5=?"); vals.append(_zip5(v))
    cols += ["updated_at=?", "edited_at=?"]; vals += [now, now]
    with connect() as conn:
        cur = conn.execute(f"UPDATE terminals SET {', '.join(cols)} WHERE sd_id=?",
                           vals + [sd_id])
        conn.commit()
        if cur.rowcount == 0:
            return None
...
def mark_pushed(sd_id: str, sd_fields: dict) -> None:
    """Record that a row's edits were successfully PUT to SuperDispatch: merge the pushed
    values into raw_json (so it becomes the new SD baseline) and clear edited_at (the row is
    back in sync, so a re-push is a no-op and the pull won't treat it as pending). Keyed on
    sd_id. `sd_fields` is keyed by SD body name (name/address/city/state/zip/notes)."""
    sd_id = (sd_id or "").strip()
    if not sd_id:
        return
    with connect() as conn:
        ...
        try:
            ...
            raw = {}
        raw.update(sd_fields)                       # new SD baseline for the pushed fields
        conn.execute("UPDATE terminals SET raw_json=?, edited_at=NULL, updated_at=? WHERE sd_id=?",
                     (json.dumps(raw, ensure_ascii=False), time.time(), sd_id))
        conn.commit()
