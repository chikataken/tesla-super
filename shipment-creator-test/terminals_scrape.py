...
def main(limit: int | None = None, resume: bool = False) -> None:
    ...
    with auth.browser_context() as ctx:
        ...
        # Preserve rows the user edited locally that haven't been pushed to SD yet, so the
        # pull never clobbers a pending local change (e.g. a contact-only edit the push can't
        # send). In the daily sync the push runs FIRST and clears edited_at on everything it
        # sent, so only genuinely-unpushed edits stay protected here.
        protected = {t["sd_id"] for t in tdb.all_terminals() if t.get("edited_at")}
        todo = [o for o in objects if o.get("guid") not in have and o.get("guid") not in protected]
        n_prot = sum(1 for o in objects if o.get("guid") in protected)
        print(f"enriching {len(todo)} terminals (skipping {len(objects)-len(todo)} cached"
              f"{f', {n_prot} locally-edited preserved' if n_prot else ''})")
