...
# --- TEST SITE flag (test.wastake.com) -----------------------------------
# When SC_TEST_MODE is truthy this instance is the read-only test build: it may READ from
# SuperDispatch (login, loadboard scan, terminal refresh) but every WRITE is blocked —
# order posting, consolidation PATCHes, and terminal push all refuse. The flag is dormant
# (off) unless the environment sets it, so promoting this code to prod changes nothing.
TEST_MODE = os.getenv("SC_TEST_MODE", "").strip().lower() in {"1", "true", "yes", "on"}
...
# prices are never sent, only the order total. Paid on 15-day terms.
