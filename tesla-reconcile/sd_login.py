...
def _signin_again(page):
    ...
    On session expiry SD sometimes shows this gate *before* the login form (no
    password field, URL not yet on /login). `exact=False` substring-matches, so it
    catches "Sign in again" / "Sign In Again" but NOT the form's plain "Sign In"
    submit (which lacks "again")."""
    for role in ("button", "link"):
        try:
            loc = page.get_by_role(role, name="Sign in again", exact=False)
...
def is_login_page(page) -> bool:
    """True if `page` is on the SD sign-in screen or a 'Sign in again' interstitial."""
    try:
        if "login" in (page.url or "").lower():
            return True
        if page.locator("input[type=password]").count() > 0:
            return True
        return _signin_again(page) is not None
    except Exception:
        return False
...
def _human_click(page, locator) -> None:
    """Move the pointer onto `locator` (jittered) and click — same care as typing."""
    try:
        box = locator.bounding_box()
        if box:
            page.mouse.move(box["x"] + box["width"] * random.uniform(0.3, 0.7),
...
def ensure_logged_in(page) -> bool:
    ...
    # Session-expiry interstitial: SD may show a "Sign in again" gate before the
    # login form. Click it carefully and wait for the form to render.
    again = _signin_again(page)
    if again is not None:
        log("sd_login: clicking the 'Sign in again' button")
        _human_click(page, again)
