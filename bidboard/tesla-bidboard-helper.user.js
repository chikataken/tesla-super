...
(function () {
  ...
  // Pickup is always 16:00Z (4 PM), USD. Pickup DATE: Fri/Sat/Sun bids -> next WEDNESDAY; otherwise today + 3 CALENDAR days, rolled forward to
  ...
  // (US state centroids): <500mi:5  500-1000:9  1000-2000:11  >=2000:12  (intra-state -> 4).
  ...
  // Earliest valid need-by across the route's VINs (midnight local), or null if none.
  function needByEta(g) { const ds = ((g.bids && g.bids.items) || []).map((b) => b.needByDate && new Date(b.needByDate)).filter((d) => d && !isNaN(d)); if (!ds.length) return null; const t = new Date(Math.min(...ds)); t.setHours(0, 0, 0, 0); return t; }
  ...
  // State unparseable (service centers, street addresses — no NA-US-XX in the name): distance is
  // unknowable and the flat 4-day fallback promised up to 4 days past Tesla's need-by; recommend
  // Tesla's own (earliest) need-by instead, floored at the pickup date so the ETA never precedes it.
